...
We can see that the best model explains approximately 81.6% of the variance in the response and has an RMSE of 34927.44
...
qplot(x=fitted, y = inp_df$SalePrice,xlab = "Fitted data", ylab = "True Response",
      main = "In-Sample comparison: Predicted Sale Price vs. True Response",alpha=0.5)
```
  
Here are the top 10 predictors defined by the variable importance []:
```{r}
var_imp = varImp(lasso_model)
var_imp$parameter = row.names(var_imp)
var_imp = var_imp[order(var_imp$Overall,decreasing = T),] 

var_imp_top10 = var_imp[1:10,]
print(var_imp_top10)
```

Neighborhood seems to be the top influencing factor along with a good/excellent swimming pool, excellent kitchen, and all-stone exterior.
...
# Add placeholder response column to maintain the structure
...
test = test[,cols] # retain only the selected columns
...
# If some factor levels do not appear in the test set, the prediction will fail
# Add these levels:

setdiff(names(input_vars_test[1,]),names(input_vars[1,]))
...
models[models$lambda == lambda.1se,]
```

The best and simplest model explains 88% of the variance of the response variable.
...
# It is crucial to set the factor levels to the same levels as in the training data
# otherwise the glmnet prediction will fail due to a different dimension of the dummy
# variable matrix
...
for(i in 1:ncol(test)) {
  if(is.factor(test[,i])) {
    levels(test[,i]) <- c(levels(train[,i]))
    }
}
